...
namespace Plugin.Managers {
    ...
    internal static class SoundManager
    {
        ...
        internal static void Initialize(PlaySoundDelegate playSound, PlayCarSoundDelegate playCarSound, int numIndices)
        {
        ...
        internal static void PlayAllCar(int soundIndex, double volume, double pitch, bool loop) {
            for (int i = 0; i < Plugin.specs.Cars; i++) {
        ...
        internal static void PlayCar(int soundIndex, double volume, double pitch, bool loop, int carIndex) {
            if (carIndex > Plugin.specs.Cars - 1) return;
            volume = volume < 0.0 ? 0.0 : volume;
            pitch = pitch < 0.0 ? 0.0 : pitch;
            if (soundIndex == -1) return;
        ...
        internal static void StopCar(int soundIndex, int carIndex) {
            ...
            if (carIndex > Plugin.specs.Cars - 1 || (soundIndex == -1 || SoundHandles[carIndex].CurrentHandles[soundIndex] == null)) return;
            SoundHandles[carIndex].CurrentHandles[soundIndex].Stop();
        ...
        internal static bool IsPlayingOnCar(int soundIndex, int carIndex) {
            return carIndex <= Plugin.specs.Cars - 1 && (soundIndex != -1 && SoundHandles[carIndex].CurrentHandles[soundIndex] != null && SoundHandles[carIndex].CurrentHandles[soundIndex].Playing);
        }
